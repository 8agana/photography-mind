...
use photography_mind::photography::DEFAULT_COMPETITION;
    ...
    match cli.command {
        ...
        Commands::List { list_command } => match list_command {
            ListCommands::Skaters { status } => {
                photography_mind::photography::commands::list_skaters(&db, &status).await?;
            }
            ListCommands::Events { competition } => {
                photography_mind::photography::commands::list_events(&db, &competition).await?;
            }
        },
        Commands::Show { show_command } => match show_command {
            ShowCommands::Event {
                ...
                photography_mind::photography::commands::show_event(
                    &db,
                    event_number,
                    split.as_deref(),
                )
        ...
        Commands::Update { update_command } => match update_command {
            UpdateCommands::Gallery {
                ...
                photography_mind::photography::commands::update_gallery(
                    &db,
                    &skater,
                    event,
                    &status,
        ...
        Commands::QuerySkater { last_name } => {
            photography_mind::photography::commands::query_skater(&db, &last_name).await?;
        }
        Commands::GetEmail { last_name } => {
            photography_mind::photography::commands::get_email(&db, &last_name).await?;
        }
        Commands::MarkSent {
            last_name,
            competition,
        ...
        Commands::SendTy {
            ...
            photography_mind::photography::commands::send_ty(&db, &last_name, &competition).await?;
        }
        Commands::RecordPurchase {
            ...
            photography_mind::photography::commands::record_purchase(
                &db,
                &last_name,
                amount,
                &competition,
        ...
        Commands::CheckStatus {
            ...
            photography_mind::photography::commands::check_status(
                &db,
                &competition,
                pending_only,
                ty_pending,
        ...
        Commands::ListEventsForSkater {
            ...
            photography_mind::photography::commands::list_events_for_skater(
                &db,
                &skater,
        ...
        Commands::CompetitionStats { comp_name } => {
            photography_mind::photography::commands::competition_stats(&db, &comp_name).await?;
        }
    }
